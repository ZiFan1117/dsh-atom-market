...
function toRecord(manifest, origin) {
    return {
...
export function isStoreRoot(dir) {
    const atomsDir = join(dir, 'atoms');
    if (!existsSync(atomsDir) || !statSync(atomsDir).isDirectory())
        return false;
    try {
...
export function readAtoms(root) {
    ...
    for (const f of files) {
        try {
            const manifest = JSON.parse(readFileSync(join(atomsDir, f), 'utf8'));
...
async function loadFromGitHub(env) {
    ...
    try {
        const listRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/atoms?ref=${branch}`, { headers });
        if (!listRes.ok) {
        ...
        for (const f of files) {
            try {
                const fileRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/atoms/${encodeURIComponent(f.name)}?ref=${branch}`, { headers });
                if (!fileRes.ok)
                    continue;
...
export function searchAtoms(records, opts) {
    ...
    const filtered = records.filter((r) => {
        if (opts.layer && r.layer !== opts.layer)
            return false;
        if (opts.verified !== undefined && r.verified !== opts.verified)
            return false;
        if (!q)
            return true;
