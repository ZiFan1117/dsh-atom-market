    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (fetched.error || !fetched.manifest) {
                return { ok: false, id: args.id, tier: rec.tier, source: rec.repo, error: fetched.error ?? '无 manifest' };
            }
            return { ok: true, id: rec.id, tier: rec.tier, source: rec.repo, manifest: fetched.manifest };
        },
    }));
    ctx.tools.register(defineTool({
        name: 'atom_validate',
