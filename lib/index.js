...
import { openStore, searchAtoms, readAtom } from './store.js';
...
export function apply(ctx) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (!rec)
                return { ok: false, id: args.id, error: `商店中找不到原子 ${args.id}` };
            const publicRec = rec;
            return { ok: true, id: rec.id, intent: rec.intent, manifest: publicRec.manifest };
        },
    }));
    ctx.tools.register(defineTool({
        name: 'atom_validate',
        ...
        async execute(args) {
            return validateManifestText(args.manifest);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'atom_draft',
